...
describe('Polar Coordinate system', () => {
    ...
    it('convert to device point', () => {
        const point = coordinate.point(Math.PI / 4, Math.sqrt(2));
        const dpoint = point.toDevice(new DeviceCoordinate(coordinate.originX, coordinate.originY));
        expect(dpoint.x).to.be.closeTo(1, MIN_ACCURACY);
        expect(dpoint.y).to.be.closeTo(-1, MIN_ACCURACY);
    });
    it('convert to cartesian point', () => {
        const point = coordinate.point(Math.PI / 4, Math.sqrt(2));
        const dpoint = point.toCartesian(new CartesianCoordinate(coordinate.originX, coordinate.originY));
        expect(dpoint.x).to.be.closeTo(1, MIN_ACCURACY);
        expect(dpoint.y).to.be.closeTo(1, MIN_ACCURACY);
    });
    it('convert to polar point', () => {
        const point = coordinate.point(Math.PI / 4, Math.sqrt(2));
        const convertedPoint = point.toPolar(new PolarCoordinatate(coordinate.originX, coordinate.originY));
